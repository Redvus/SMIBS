(function () {
    function grantProjectGridder() {
        $("#grantProjectGridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function corporateProjectGridder() {
        $("#corporateProjectGridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function contests2019Gridder() {
        $("#contests2019Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function contests2020Gridder() {
        $("#contests2020Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function exhibition2019Gridder() {
        $("#exhibition2019Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function exhibition2020Gridder() {
        $("#exhibition2020Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function conference2019Gridder() {
        $("#exhibition2020Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    function conference2020Gridder() {
        $("#exhibition2020Gridder").gridderExpander({
            scroll: true,
            scrollOffset: 75,
            scrollTo: "panel", // panel or listitem
            animationSpeed: 900,
            animationEasing: "easeInOutExpo",
            showNav: true, // Show Navigation
            nextText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_next</title><polyline points="0 0 100 100 0 200"/></svg>',
            prevText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 100 200"><title>bm_prev</title><polyline points="100 200 0 100 100 0"/></svg>',
            closeText:
                '<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 200.71 200.71"><title>bm_close</title><line x1="200" y1="200" x2="0" y2="0"/><line x1="200" y1="0" x2="0" y2="200"/></svg>',
        });
    }

    /*==============================================
    =                   Magnific                   =
    ==============================================*/

    function magnificVideo() {
        $(".main-content__hero").magnificPopup({
            delegate: "a.main-content__hero-video", // child items selector, by clicking on it popup will open
            type: "iframe",
            iframe: {
                markup:
                    '<div class="mfp-iframe-scaler">' +
                    '<div class="mfp-close"></div>' +
                    '<iframe class="mfp-iframe" frameborder="0" allowfullscreen></iframe>' +
                    "</div>", // HTML markup of popup, `mfp-close` will be replaced by the close button

                patterns: {
                    youtube: {
                        index: "youtube.com/", // String that detects type of video (in this case YouTube). Simply via url.indexOf(index).

                        id: "v=", // String that splits URL in a two parts, second part should be %id%
                        // Or null - full URL will be returned
                        // Or a function that should return %id%, for example:
                        // id: function(url) { return 'parsed id'; }

                        src: "//www.youtube.com/embed/%id%?autoplay=1", // URL that will be set as a source for iframe.
                    },
                    vimeo: {
                        index: "vimeo.com/",
                        id: "/",
                        src: "//player.vimeo.com/video/%id%?autoplay=1",
                    },
                    gmaps: {
                        index: "//maps.google.",
                        src: "%id%&output=embed",
                    },

                    // you may add here more sources
                },
                srcAction: "iframe_src", // Templating object key. First part defines CSS selector, second attribute. "iframe_src" means: find "iframe" and set attribute "src".
            },
        });
    }

    function magnificGallery() {
        $(".main-content__gallery").magnificPopup({
            type: "image",
            delegate: "a",
            mainClass: "mfp-with-zoom", // this class is for CSS animation below

            zoom: {
                enabled: false, // By default it's false, so don't forget to enable it

                duration: 300, // duration of the effect, in milliseconds
                easing: "ease-in-out", // CSS transition easing function

                // The "opener" function should return the element from which popup will be zoomed in
                // and to which popup will be scaled down
                // By defailt it looks for an image tag:
                opener: function (openerElement) {
                    // openerElement is the element on which popup was initialized, in this case its <a> tag
                    // you don't need to add "opener" option if this code matches your needs, it's defailt one.
                    return openerElement.is("img")
                        ? openerElement
                        : openerElement.find("img");
                },
            },
        });
    }

    /*============  End of Magnific  =============*/

    function initPage() {
        // grantProjectGridder();
        // corporateProjectGridder();
        // exhibition2019Gridder();
        // exhibition2020Gridder();
        // contests2019Gridder();
        // contests2020Gridder();
        // conference2019Gridder();
        // conference2020Gridder();
        magnificVideo();
        magnificGallery();
    }

    function initPageMobile() {
        magnificVideo();
        magnificGallery();
    }

    if (document.body.clientWidth > 768 || screen.width > 768) {
        initPage();
    } else {
        initPageMobile();
    }
})(jQuery);
